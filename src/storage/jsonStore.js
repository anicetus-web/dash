...
export const EMPTY_CACHE = {
  ...
  calls: [],
  // Отметка последней успешно полученной истории на сущность: { [id]: updatedAt опрошенной версии }.
  // Сейчас не наполняется: история стадий портала забирается ОДНИМ общим журналом
  // (`/v1/stage-history`), а не поштучным опросом сущностей, и пропускать в нём
  // нечего. Ключ оставлен в форме снимка, чтобы уже записанные файлы дочитывались
  // без миграции, и на случай возврата к поштучной выборке.
