...
export async function fetchBitrixSnapshot(options = {}) {
  ...
  const [companyResult, dealResult, usersRaw, dealFieldsBody, stageHistory, callsRaw] = await Promise.all([
    ...
    // Звонки идут ВНЕ общей очереди слотов. В очереди их ветка голодает:
    // история стадий занимает почти все слоты своими 347 страницами, и звонки
    // выбирались со скоростью около страницы в три секунды — не укладываясь
    // ни в один разумный бюджет. Ветка ограничена по времени, поэтому лишний
    // одновременный запрос к порталу не может ни затянуть, ни подвесить заход.
    fetchOptional(
