...
function buildWarnings(slice, rows, options = {}) {
  ...
  if (isDemo) {
    // Демо-снимок при НАСТРОЕННОМ портале — это не «портал не подключён», а
    // «снимок ещё не собран»: развёртывание очищает папку данных, и до конца
    // первой синхронизации показывать нечего. Прежний текст в этом случае врал
    // и отправлял искать несуществующую поломку интеграции.
    const portalConfigured = options.dataSource === 'bitrix';
    warnings.push({
      code: WARNING_CODES.demoData,
      message: portalConfigured
        ? 'Портал Битрикс24 подключён, но снимок ещё не собран — пока показаны демонстрационные цифры. Первая синхронизация после перезапуска занимает несколько минут.'
        : 'Демонстрационные данные: цифры сгенерированы, портал Битрикс24 не подключён.'
    });
  }
