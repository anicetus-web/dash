...
// Первый ID поля, которое портал может отдать как множественное.
//
// Множественное перечисление Битрикса приезжает МАССИВОМ даже с одним значением: «база»
// приходит как `[493]`, а не `493`. `idOf` на массиве честно возвращает '' — то есть
// значение теряется молча, без ошибки. Ровно так «база» оказалась пустой у всех записей
// первого боевого прогона. Скаляр обрабатывается тем же путём, поэтому вызывающему коду
// не нужно знать, множественное поле или нет. Пустой массив — это «не заполнено», а не ID.
export function firstIdOf(value) {
  return idsOf(value)[0] ?? '';
}
